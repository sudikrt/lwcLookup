...
export default class Lookup extends LightningElement {
        ...
        this.searchResults = resultsLocal.map ((result) => {
            ...
            if (this.searchTerm.length > 0) {
                ...
                result.titleFormatted = result.title
                    ...
                    : result.title;
                result.subtitleFormatted = result.subtitle && result.subtitle !== null
    ...
    updateSearchTerm (newSearchTerm) {
        ...
        //Compare clean new search Term and with current one and abort if identical
        const newCleanSearchTerm = newSearchTerm.trim().replace (/\*/g, '').toLowerCase ();
        if (this.searchTerm === newCleanSearchTerm) {
            return;
        } 
        ...
        //Ignore search terms that are small
        ...
        this.searchThrottlingTimeout = setTimeout (() => {
            //Send the search event if search term is long enough
            if (this.cleanSearchTerm.length >= MINIMAL_SEARCH_TERM_LENGTH) {
                ...
                const searchEvent = new CustomEvent ('search', {
                    detail : {
                        ...
                        selectedIds : this.curSelection.map (ele => ele.id) 
                    }
                });
    ...
    handleResultClick (event) {
        ...
        const newSelections = [...this.curSelection];
        newSelections.push (selectedItem);
        this.curSelection = newSelections;
        this.isDirty = true;
    ...
    handleFocus () {
        //Prevent if selection is not allowed 
        if (!this.isSelectionAllowed ()) {
            return;
        }
        this.hasFocus = true;
    ...
    handleBlur () {
        //prevent if selection not allowed 
        if (!this.isSelectionAllowed ()) {
            return;
        }
    ...
    handleRemoveSelectedItem (event) {
        ...
        this.curSelection = this.curSelection.filter (ele => ele.id !== recordId);
        this.isDirty = true;
        //notify parent component that selection has changed
        this.dispatchEvent (new CustomEvent ('selectionchange'));
    }
